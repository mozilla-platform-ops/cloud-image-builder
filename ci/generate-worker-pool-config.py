...
#azureDeployment = 'azure_alpha' if currentEnvironment == 'production' else 'azure_gamma'
azureDeployment = 'azure_gamma'
...
workerPool = {
    ...
    'launchConfigs': list(filter(lambda x: x['storageProfile']['imageReference']['id'] is not None and x['location'] in poolConfig['locations'] and x['location'] in includeRegions, map(lambda x: {
        ...
        'diagnosticsProfile': {
            'bootDiagnostics': {
                'storageUri': 'https://{}.blob.core.windows.net'.format(x['storage']),
                'enabled': True
