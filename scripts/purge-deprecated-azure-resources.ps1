...
Get any resource groups that match the following:

Contains "-us-"
Ends with '-gecko-1','-gecko-3','-gecko-t','-relops','-mpd001-1','-mpd001-3'
...
$ConnectSplat = @{
  ...
  Credential = (New-Object System.Management.Automation.PSCredential($secret.azure_gamma.app_id, (ConvertTo-SecureString -String $secret.azure_gamma.password -AsPlainText `-Force)))
  TenantID = ""
  SubscriptionId = ""
}
...
Set-AzContext -TenantDd "" -SubscriptionId "" | Out-Null
