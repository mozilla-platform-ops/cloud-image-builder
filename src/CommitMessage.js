      ...
      <div>
        {
          this.props.message.filter(line => (
            ...
            &&
            !line.match((new RegExp ('^(pool-deploy|overwrite-machine-image|disable-cleanup|purge-taskcluster-resources|no-ci|no-taskcluster-ci|no-travis-ci)$', 'i')))
          )).map((line, lI) => (
            (lI === 0)
        ...
        }
        {
          (this.props.message.some(line => (
            ...
            ||
            line.match(/^(pool-deploy|overwrite-machine-image|disable-cleanup|purge-taskcluster-resources|no-ci|no-taskcluster-ci|no-travis-ci)$/i)
          )))
            ? (
                (this.props.message.filter(line => (line.match(/^(pool-deploy|overwrite-machine-image|disable-cleanup|purge-taskcluster-resources|no-ci|no-taskcluster-ci|no-travis-ci)$/i)))).map(instruction => (
                  <Badge
                    style={{ marginRight: '0.7em' }}
                    variant={(['pool-deploy', 'overwrite-machine-image', 'disable-cleanup', 'purge-taskcluster-resources'].includes(instruction)) ? 'primary' : 'dark'}>
                    {instruction}
                  </Badge>
                ))
              )
